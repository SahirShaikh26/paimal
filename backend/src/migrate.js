// Schema migration, split out of server.js so it runs deliberately rather than
// on every boot — serverless invocations must never race each other running DDL.
const db = require('./db');
...
async function migrate() {
  const stmts = [
    ...
    `ALTER TABLE customers ADD COLUMN IF NOT EXISTS contact_email VARCHAR(150)`,
